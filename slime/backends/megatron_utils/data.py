...
def _get_capped_partitions(seqlen_list: Sequence[int], num_partitions: int, max_tokens: int) -> list[list[int]]:
    """First-fit partitioning that returns exactly ``num_partitions`` non-empty bins.

    ``num_partitions`` is synchronized across data-parallel ranks. Some ranks may
    need fewer bins than the DP-wide maximum, but Megatron still executes all
    micro-batches on every rank. Therefore every returned partition must be
    non-empty; an empty partition would later become an empty token list in
    ``get_batch``.

    We first use the same first-fit order as
    ``get_minimum_num_micro_batch_size`` to get a cap-valid packing. If that
    packing uses fewer bins than the DP-wide ``num_partitions``, we split
    existing multi-sample bins until the counts match. Splitting can only reduce
    per-bin token sums, so it preserves the token cap.
    """
    if num_partitions <= 0:
        raise ValueError(f"num_partitions must be positive, got {num_partitions}")
    if num_partitions > len(seqlen_list):
        raise ValueError(
            f"Cannot create {num_partitions} non-empty micro-batches from only "
            f"{len(seqlen_list)} local samples. Reduce DP-wide micro-batches or "
            "increase the local batch size."
        )
    ...
    if len(partitions) > num_partitions:
        raise ValueError(
            f"Could not partition {len(seqlen_list)} samples under max_tokens_per_gpu * cp_size "
            f"({max_tokens}) using {num_partitions} micro-batches. Increase --max-tokens-per-gpu."
        )

    while len(partitions) < num_partitions:
        split_idx = max(range(len(partitions)), key=lambda i: len(partitions[i]))
        if len(partitions[split_idx]) <= 1:
            raise ValueError(
                f"Cannot create {num_partitions} non-empty micro-batches from this packing; "
                "all existing micro-batches contain one sample."
            )
        moved_idx = partitions[split_idx].pop()
        sums[split_idx] -= seqlen_list[moved_idx]
        partitions.append([moved_idx])
        sums.append(seqlen_list[moved_idx])
