...
async def generate_and_rm(
    ...
    if isinstance(sample, list):
        ...
        has_eval_rm_type = any(
            isinstance(sample.metadata, dict) and sample.metadata.get("rm_type") for sample in samples_need_reward
        ) or bool(getattr(args, "eval_rm_type", None))
        rm_kwargs = {"evaluation": True} if evaluation and has_eval_rm_type else {}
        with trace_span(samples_need_reward, "reward_model"):
            rewards = await batched_async_rm(args, samples_need_reward, **rm_kwargs)
        ...
        if sample.reward is None:
            has_eval_rm_type = (isinstance(sample.metadata, dict) and sample.metadata.get("rm_type")) or bool(
                getattr(args, "eval_rm_type", None)
            )
            rm_kwargs = {"evaluation": True} if evaluation and has_eval_rm_type else {}
            with trace_span(sample, "reward_model"):
                sample.reward = await async_rm(args, sample, **rm_kwargs)
...
async def generate_and_rm_group(
    ...
    if not state.aborted and args.group_rm:
        has_eval_rm_type = any(
            isinstance(sample.metadata, dict) and sample.metadata.get("rm_type") for sample in group
        ) or bool(getattr(args, "eval_rm_type", None))
        rm_kwargs = {"evaluation": True} if evaluation and has_eval_rm_type else {}
        with trace_span(group, "group_reward_model"):
            rewards = await batched_async_rm(args, group, **rm_kwargs)
...
async def eval_rollout_single_dataset(
    ...
    cache_key = dataset_cfg.cache_key + (args.hf_checkpoint, args.apply_chat_template, args.disable_thinking)
    ...
    for sample in data:
        if isinstance(sample.reward, dict):
            ...
            sample.metadata.setdefault("raw_reward", raw_reward)
            if reward_key is not None:
                sample.reward = raw_reward[reward_key]
            elif "score" in raw_reward:
                sample.reward = raw_reward["score"]
            elif "reward" in raw_reward:
                sample.reward = raw_reward["reward"]
            else:
                raise TypeError(
                    "Eval reward is a dict, but no scalar reward key was provided. "
                    f"Set --eval-reward-key or include a 'score'/'reward' field. Got keys: {list(raw_reward.keys())}"
                )
    return {
        dataset_cfg.name: {
