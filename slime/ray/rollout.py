...
def _compute_zero_std_metrics(args, all_samples: list[Sample]):
    ...
    def _get_numeric_rewards(samples: list[Sample]):
        rewards = []
        for sample in samples:
            try:
                ...
                return None
            if isinstance(reward, numbers.Real):
    ...
    for group in all_sample_groups.values():
        rewards = _get_numeric_rewards(group)
        if rewards is None:
            continue
        if len(rewards) > 0 and all(rewards[0] == reward for reward in rewards):
