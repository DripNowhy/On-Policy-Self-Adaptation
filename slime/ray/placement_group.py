...
from slime.backends.megatron_utils.opsa import requires_reference_model
...
def _create_placement_group(num_gpus):
    ...
    for i in range(num_bundles):
        ...
        logger.info(
            f"  bundle {i:4}, actual_bundle_index: {actual_bundle_index:4}, node: {gpu_ids[actual_bundle_index][0]}, gpu: {gpu_ids[actual_bundle_index][1]}"
        )
...
def create_training_models(args, pgs, rollout_manager):
    ...
    start_rollout_ids = ray.get(
        actor_model.async_init(
            args,
            role="actor",
            with_ref=requires_reference_model(args),
