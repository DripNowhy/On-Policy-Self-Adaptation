...
async def async_rm(args, sample: Sample, **kwargs):
    ...
    eval_rm_type = getattr(args, "eval_rm_type", None) if evaluation else None
    rm_type = (metadata.get("rm_type") or eval_rm_type or args.rm_type or "").strip()
    use_eval_rm_type = evaluation and bool(metadata.get("rm_type") or eval_rm_type)
    if args.custom_rm_path is not None and not use_eval_rm_type:
...
async def batched_async_rm(
    ...
    **kwargs,
    ...
    eval_rm_type = getattr(args, "eval_rm_type", None) if evaluation else None
    use_eval_rm_type = evaluation and (
        bool(eval_rm_type)
        or any(isinstance(sample.metadata, dict) and sample.metadata.get("rm_type") for sample in samples)
    )
    if args.custom_rm_path is not None and not use_eval_rm_type:
