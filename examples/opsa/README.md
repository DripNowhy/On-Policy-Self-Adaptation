...
OPSA (On-Policy Self-Advantage) trains only the actor's lowest-log-probability
...
from a separately converted Megatron checkpoint. From the repository root,
select the model definition and convert it:
...
`--ray-address` is omitted and stops only that cluster on exit. To use a cluster
you already manage, pass its dashboard URL:
