...
This `slime-opsa` tree was prepared from the tracked `slime/` contents at local
repository commit `594c562`. The existing working tree was not copied wholesale;
the OPSA implementation and its focused ablations were selected explicitly.
...
machine-local paths are intentionally excluded. The source remains under the
